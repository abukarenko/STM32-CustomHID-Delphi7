...
void write_buffer() {
	...
	HAL_SPI_Transmit(&hspi1, (uint8_t*)v_buffer, buffer_counter, 10);
	buffer_counter = 0;
}
...
void lcd_init_command_list(void)
{
	...
	lcd_write_command(0x20); // Invert OFF
 ...
 */
void fill_rectangle(unsigned int x1, unsigned int y1,
                    unsigned int x2, unsigned int y2,
                    unsigned int colour)
{
    uint8_t r = ((colour >> 11) & 0x1F) << 3;
    uint8_t g = ((colour >> 5)  & 0x3F) << 2;
    uint8_t b = ((colour >> 0)  & 0x1F) << 3;

    static uint8_t line_buffer[480 * 3];
    ...
    set_draw_window(x1, y1, x2 - 1, y2 - 1);
    ...
    {
        HAL_SPI_Transmit(&hspi1, line_buffer, width * 3, 100);
    }
